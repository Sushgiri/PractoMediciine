...
@Configuration
@EnableWebSecurity
@EnableMongoRepositories(basePackages = "com.product.shopping.repository")
public class SecurityConfig  extends WebSecurityConfigurerAdapter {
...
//
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                .csrf().disable()
                .authorizeRequests()
                .antMatchers(HttpMethod.GET,"/shopping/**").hasRole("ADMIN")
                .antMatchers(HttpMethod.DELETE,"/shopping/**").hasRole("ADMIN")
                .antMatchers(HttpMethod.PUT,"/shopping/**").hasRole("USER")
                .antMatchers(HttpMethod.POST,"/shopping/**").hasRole("USER")
                .antMatchers(HttpMethod.GET,"/comment/**").hasRole("ANALYST")
                .antMatchers(HttpMethod.DELETE,"/comment/**").hasRole("COMMENTOR")
//
                .anyRequest()
